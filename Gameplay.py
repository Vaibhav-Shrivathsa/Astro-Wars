    ...
    def pos_update(self, dt, stage):
        ...
        if stage.outer_boundary is not None:
            if ship_collider.is_colliding_with_inverted_rect(stage.outer_boundary.collider):
                self.pos, self.vel = ship_collider.fix_collision_with_inverted_rect(stage.outer_boundary.collider, self.vel)
...
def game_match_phase(game):
    ...
    fps = UITools.Text(str("{:.0f}".format(1 / dt)), UI.WHITE, Vector2(0.09, 0.09), 0.04)
    fps.render(game)
